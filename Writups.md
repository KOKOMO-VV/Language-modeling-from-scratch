...
- Token embedding: `vocab_size` $\times$ `d_model` $= 50257\times1600 = 80{,}411{,}200$
...
- Token embedding $=$ `vocab_size` $\times$ `d_model` $= 10000\times512 = 5{,}120{,}000$
...
- LM head $=$ `vocab_size` $\times$ `d_model` $= 10000\times512 = 5{,}120{,}000$
...
QKV projection, the weighted sum (after `softmax(QK^T/√d_k)@V`), and the output projection: since $h\times d_k = d_{\text{model}}$ (splitting into multiple heads just breaks up the last dimension without changing the total element count), all three have a total element count equivalent to `(batch_size, context_length, d_model)`.

$QK^\top$ and softmax are the exception: these must be counted as `batch_size` $\times$ `num_heads` $\times$ `context_length`$^2$, not as `(batch_size, context_length, d_model)` — this matters because `context_length` appears squared here, and its magnitude is usually much larger than the other terms.

The FFN needs its shape traced step by step through the SwiGLU formula $\text{FFN}(x)=W_2(\text{SiLU}(W_1x)\odot W_3x)$: $W_1x$ maps from $d_{\text{model}}$ to $d_{ff}$, giving shape `(batch_size, context_length, d_ff)`; SiLU acts elementwise, leaving the shape unchanged; $W_3x$ is likewise a $d_{\text{model}}\to d_{ff}$ mapping, also shape $d_{ff}$; the elementwise product of the two keeps shape $d_{ff}$; only after $W_2(\cdot)$ maps $d_{ff}$ back down to $d_{\text{model}}$ does the output shape return to `(batch_size, context_length, d_model)` — so the FFN contributes 4 intermediate tensors of size $d_{ff}$ plus 1 output tensor of size $d_{\text{model}}$.

The final RMSNorm is counted as `(batch_size, context_length, d_model)`; the LM head and cross-entropy are counted as `(batch_size, context_length, vocab_size)`.

Listed out (each figure is the element count per unit of batch_size):

- Per-layer's 2 RMSNorms: `context_length` $\times$ `d_model` $\times2 = 262{,}144$
- QKV projection: `context_length` $\times$ `d_model` $\times3 = 393{,}216$
- $QK^\top$: `num_heads` $\times$ `context_length`$^2$ $= 1{,}048{,}576$
- softmax: `num_heads` $\times$ `context_length`$^2$ $= 1{,}048{,}576$
- Weighted sum: `context_length` $\times$ `d_model` $= 131{,}072$
- Output projection: `context_length` $\times$ `d_model` $= 131{,}072$
- FFN (4 intermediates of size $d_{ff}$ + 1 output of size $d_{\text{model}}$): $1{,}507{,}328$
- Final RMSNorm: $131{,}072$
- LM head: `context_length` $\times$ `vocab_size` $= 2{,}560{,}000$
- Cross-entropy: `context_length` $\times$ `vocab_size` $= 2{,}560{,}000$

Total: $23{,}339{,}008\times$ `batch_size` activation elements.
